...
We used MetaDrive~\cite{li2021metadrive}~\ref{fig:metadrive} for traffic simulator which
...
other objects or vehicles. We run the traning fpr both algorithms for 5 tryouts and plot the average for each evaluation metric. The PPO hyperparameters are shown in table~\ref{tab:ppo_hyperparameters}.
\begin{table}[htbp]
    \centering
    \begin{tabular}{|l|c|c|}
        \hline
...
performance after the update of the value function. This is
...
\begin{figure}[H]
    ...
    \caption{Comparision of single agent traning by average success rate betwwen
PPO Beta (Ours) and standard PPO Gaussian.}\label{fig:single_agent_success_rate}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{Comparision of single agent traning by average safety rate betwwen
PPO Beta (Ours) and standard PPO Gaussian.}\label{fig:figure6}
\end{figure}
...
shown in~\ref{fig:figure6}, both success rates were below 50%. This is
because the single-agent environment is inherently difficult,
as the training agent cannot coordinate its actions with the
other agents.
Despite the difficulty of the environment, PPO-
Beta was able to learn to dodge the incoming vehicle better
